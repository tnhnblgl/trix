...
- **The pipe is byte-mode** (`PIPE_TYPE_BYTE | PIPE_READMODE_BYTE`, `pipe.rs:48`) with newline framing, so the *protocol* needs nothing but `std::fs::File` and `BufRead`.

  **CORRECTED AFTER TASK 5.** This plan originally claimed `trix-ui` would need **no `windows` crate** for the socket. That was wrong, and it cost a Critical defect: every `trix_call` from the webview hung forever. A pipe opened without `FILE_FLAG_OVERLAPPED` gets `FO_SYNCHRONOUS_IO` on its *file object*, and the I/O manager serializes every operation on that object. `File::try_clone` is `DuplicateHandle`, which **shares** the file object — so the reader thread parked in a blocking `read_line` blocks the request `write` issued on the "other" handle, and the request never leaves. The daemon hit the identical wall in plan 3 and was fixed there first.

  So `trix-ui` **does** depend on `windows`, narrowly (`Win32_Foundation`, `Win32_System_Pipes`), for `PeekNamedPipe` and nothing else. The fix is `crates/trix-ui/src/pipe_reader.rs`: never park in a blocking read — poll on a 25 ms interval and read only once bytes are available, so the thread holds no I/O lock while it waits. This is the daemon's own proven pattern (`trix-daemon/src/pipe.rs`), and `windows` is not `trix-core`, so spec §3.2 is untouched.
...
```
crates/trix-ui/
  Cargo.toml              tauri, tauri-plugin-single-instance, trix-proto, serde_json, anyhow, windows
  ...
  src/pipe_reader.rs      PeekingPipeReader: the non-blocking read that keeps
                          the write from deadlocking behind it. The crate's
                          only Win32 code.                                    [tested]
  src/daemon.rs           discovery, launch, reconnect supervisor             [tested]
  src/commands.rs         the Tauri command surface the webview may call
  tests/pipe_roundtrip.rs the transport proof, over a private stub pipe
  web/
...
//! (`PIPE_TYPE_BYTE | PIPE_READMODE_BYTE`) with newline framing, so the
//! framing needs nothing platform-specific. Reading it without deadlocking
//! does — see `pipe_reader.rs`, which is what `daemon.rs` hands to `start`.
