...
must stop closing the dialog itself. `Modal` owns Escape now, and both
handlers sit on `window`, where `stopPropagation` does not stop a sibling
listener on the same target. Two handlers closing the same dialog is
harmless today and a trap the moment either grows. Replace the branch with:
...
```ts
    ...
    // Escape is deliberately not handled: `Modal` owns closing itself, and
    // both handlers are on `window`, so acting on it here would just close
