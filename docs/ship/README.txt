...
----------------------------
  ...
  * Trimming has arrived, fast mode only. Open a clip, set an in and an out
    ...
    second and loses no quality. The in point snaps back to the previous
    keyframe -- about once a second in clips Trix recorded, further back in
    an .mp4 from somewhere else; the out point does not move. Frame-accurate
    trimming, for landing on an exact frame, is not in yet.
